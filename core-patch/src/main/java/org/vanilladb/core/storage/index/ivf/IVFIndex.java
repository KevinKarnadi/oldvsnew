...
public class IVFIndex extends Index {
    ...
    private static final int MIN_CENTROID_COUNT;
    ...
    static {
        CENTROID_COUNT = CoreProperties.getLoader().getPropertyAsInteger(
                IVFIndex.class.getName() + ".CENTROID_COUNT", 100);
        MAX_CENTROID_COUNT = CoreProperties.getLoader().getPropertyAsInteger(
                IVFIndex.class.getName() + ".MAX_CENTROID_COUNT", 50);
        MIN_CENTROID_COUNT = CoreProperties.getLoader().getPropertyAsInteger(
                IVFIndex.class.getName() + ".MIN_CENTROID_COUNT", 20);
        MOVE_DIST_THRESHOLD = CoreProperties.getLoader().getPropertyAsInteger(
                IVFIndex.class.getName() + ".MOVE_DIST_THRESHOLD", 1000);
        MAX_ITERATIONS = CoreProperties.getLoader().getPropertyAsInteger(
                IVFIndex.class.getName() + ".MAX_ITERATIONS", 15);
        REINDEX_THRESHOLD = CoreProperties.getLoader().getPropertyAsInteger(
                IVFIndex.class.getName() + ".REINDEX_THRESHOLD", 50000);
    }

    TableInfo tiCentroid;
    List<VectorConstant> centroids;
    List<List<IVFVectorRecordId>> vectors;
    ...
    public void beforeFirst(SearchRange searchRange) {
        ...
        var distPQ = new PriorityQueue<IVFDist>((a, b) -> {
        ...
        while (rfCentroid.next()) {
            ...
            distPQ.add(new IVFDist((int) idx.asJavaVal(), euclideanFn.distance(vector), (int) itemsCount.asJavaVal()));
        }
        
        // find closests clusters' centroids
        int totalItemCount = 0;
        while (distPQ.size() > 0 && totalItemCount < MIN_CENTROID_COUNT || closestCentroidIndices.size() < MAX_CENTROID_COUNT) {
            IVFDist dis = distPQ.poll();
            closestCentroidIndices.add(dis.getIdx());
            totalItemCount += dis.getItemCount();
//            System.out.printf("%d, %f | ", dis.getIdx(), lastDist);
        }

//        System.out.printf("size=%d\n", closestCentroidIndices.size());
        
        // ensure there is at leas 1 centroid
    ...
    public boolean next() {
        ...
        boolean hasNext = recordFile.next();

        if (!hasNext && !recordFiles.isEmpty()) {
            recordFile = recordFiles.remove(0);
            hasNext = recordFile.next();
        }
...
//        System.out.println("inserting " + insert_count);
        ...
        if (insert_count != 0 && insert_count % REINDEX_THRESHOLD == 0) { // Reindex centroids
            reindexCentroids();
        }
    }
    ...
    public void delete(SearchKey key, RecordId dataRecordId, boolean doLogicalLogging) {
        ...
        while (rfCentroid.next()) {
            ...
            if (dist < minDist) {
                minDist = dist;
                closestCentroidIndex = (int) idx.asJavaVal();
            }
        }
    ...
    private void reindexCentroids() {
        ...
        do {
            ...
            assignVectors();
            iteration++;
        } while (totalCentroidMoveDist > MOVE_DIST_THRESHOLD && iteration < MAX_ITERATIONS);
    ...
    private void deleteVectorsRF() {
        ...
        while (rfCentroid.next()) {
            ...
            recordFile.beforeFirst();
            while (recordFile.next()) {
                vectors.get(centroid_idx).add(new IVFVectorRecordId(
...
//        System.out.println("totalCentroidMoveDist = " + totalCentroidMoveDist);
    }

    // Assign each vectors to nearest centroid
    private void assignVectors() {
        List<List<IVFVectorRecordId>> newClusteredVectors = new ArrayList<>(CENTROID_COUNT);
        for(int i = 0; i < CENTROID_COUNT; i++) {
        ...
        for(int i = 0; i < CENTROID_COUNT; i++) {
            for (int j = 0; j < vectors.get(i).size(); j++) {
                IVFVectorRecordId vr = vectors.get(i).get(j);
    ...
    private void writeVectorsRF() {
        ...
        for(int i = 0; i < CENTROID_COUNT; i++) {
            ...
            for (int j = 0; j < vectorCluster.size(); j++) {
                IVFVectorRecordId vr = vectorCluster.get(j);
                recordFile.insert();
