...
public class IVFFlatIndex extends Index {
    ...
    static {
        CENTROID_COUNT = CoreProperties.getLoader().getPropertyAsInteger(
                IVFFlatIndex.class.getName() + ".CENTROID_COUNT", 100);
        ...
        MOVE_DIST_THRESHOLD = CoreProperties.getLoader().getPropertyAsInteger(
                IVFFlatIndex.class.getName() + ".MOVE_DIST_THRESHOLD", 1000);
        MAX_ITERATIONS = CoreProperties.getLoader().getPropertyAsInteger(
                IVFFlatIndex.class.getName() + ".MAX_ITERATIONS", 15);
        REINDEX_THRESHOLD = CoreProperties.getLoader().getPropertyAsInteger(
                IVFFlatIndex.class.getName() + ".REINDEX_THRESHOLD", 50000);
    }
    ...
    public void beforeFirst(SearchRange searchRange) {
        ...
        PriorityQueue<IVFFlatDist> distPQ = new PriorityQueue<>(Comparator.comparingDouble(IVFFlatDist::getDist));
        ...
        while (distPQ.size() > 0 && (totalItemCount < MIN_VECTOR_COUNT || closestCentroidIndices.size() < MAX_CENTROID_COUNT)) {
